...
class HtmlUtils {
	...
	SpanWithColor(text, color, fontWeight := "normal", fontFamily := "Consolas") {
		return Format("<span style=""color: {2:s}; font-weight: {3:s}; font-family: {4:s};"">{1:s}</span>", text, color, fontWeight, fontFamily)
	}
	
	CenteredHeader(text, emSize := 2) {
		return Format("<center><h1 style=""font-family: 'starcraftregular', sans-serif; font-size: {2:i}em; color: yellow;"">{1:s}</h1></center>", text, emSize)
	}
}
